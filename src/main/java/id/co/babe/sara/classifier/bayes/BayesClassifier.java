package id.co.babe.sara.classifier.bayes;
...
import id.co.babe.sara.classifier.Classification;
import id.co.babe.sara.classifier.Classifier;

/**
 * A concrete implementation of the abstract Classifier class.  
 * The Bayesian classifier implements a naive Bayes approach 
 * Classifying a given set of features: 
 * classify(feat1,...,featN) = argmax( P(cat) * PRODUCT(P(featI|cat)))
 *
 *
...
public class BayesClassifier<T, K> extends Classifier<T, K> {
    ...
    /**
     * Calculates product of all feature probabilities: PRODUCT(P(featI|cat))
     *
    ...
    /**
     * Calculates probability features can be classified in category
     *
     * @param features The set of features to use.
     * @param category The category to test for.
     * @return The probability that features can be classified in category
     */
    public double categoryProbability(Collection<T> features, K category) {
    ...
    public double classifyScore(Collection<T> features) {
        SortedSet<Classification<T, K>> probabilites =
                this.categoryProbabilities(features);

        if (probabilites.size() > 0) {
            return probabilites.last().getProbability();
        }
        return 0;
    }
